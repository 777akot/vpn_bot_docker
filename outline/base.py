...
class OutlineManager:
    ...
    async def request(self, url: str, method: str, task: str = 'DELETE', **kwargs) -> Union[List, Dict, bool]:
        """
        ...
        return await make_request(await self.get_session(), url, method, task, timeout=self.timeout, **kwargs)

    async def create_key(self, api_key: str):
        return await self.request(api_key, Methods.KEYS, 'CREATE')

    async def delete_key(self, api_key: str, key_id: int):
        return await self.request(api_key, f"{Methods.KEYS}/{key_id}", "DELETE")
    
    async def set_data_limit(self, api_key: str, key_id: int):
        data = {"limit": {"bytes": 0}}
        return await self.request(api_key, f"{Methods.KEYS}/{key_id}/data-limit","SET_LIMIT",json=data)
